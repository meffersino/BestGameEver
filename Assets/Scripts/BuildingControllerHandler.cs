...
public class BuildingControllerHandler : MonoBehaviour {
    ...
    void InteractWithBoard(int action)
    {
        ...
        if (Physics.Raycast(ray,out hit))
        {
            ...
            {
                if (action == 0 && board.CheckForBuildAtPosition(gridPosition) == null)
                {
                ...
                else if(action == 1 && board.CheckForBuildAtPosition(gridPosition) != null)
                {
                    city.DepositCash (+board.CheckForBuildAtPosition(gridPosition).cost);
